...
class PhotoAlbumUtil: NSObject {
    ...
    //保存图片到相册
    class func saveVideoInAlbum(videoUrl: URL, albumName: String = "",
        ...
        if albumName.isEmpty {
            ...
            if assetAlbum == nil {
                PHPhotoLibrary.shared().performChanges({
                    ...
                    self.saveVideoInAlbum(videoUrl: videoUrl, albumName: albumName, completion: completion)
                })
                return
            }
        }
        ...
        PHPhotoLibrary.shared().performChanges({
            //添加的相机胶卷
            let result = PHAssetChangeRequest.creationRequestForAssetFromVideo(atFileURL: videoUrl)
            //是否要添加到相簿
            if !albumName.isEmpty {
